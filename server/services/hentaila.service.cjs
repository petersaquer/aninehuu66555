...
function parseSvelteKitNodes(nodes) {
  ...
  for (const node of nodes) {
    if (!node || node.type !== "data" || !Array.isArray(node.data) || node.data.length === 0) { results.push(null); continue; }
    const schema = node.data[0];
    const values = node.data;
    if (typeof schema === "object" && schema !== null) results.push({ schema, values });
...
function resolveNodeValue(nodeData, schemaKey) {
  if (!nodeData || !nodeData.schema || !Array.isArray(nodeData.values)) return null;
  const idx = nodeData.schema[schemaKey];
  if (typeof idx === "number" && idx >= 0 && idx < nodeData.values.length) return nodeData.values[idx];
  return null;
}
...
function resolveItemFromData(dataValues, schemaIndex) {
  ...
  for (const [key, valueIdx] of Object.entries(itemSchema)) {
    if (typeof valueIdx === "number" && valueIdx < dataValues.length) item[key] = dataValues[valueIdx];
  }
  return item;
}
...
function resolveResultsArray(dataValues, refsArray) {
  ...
  return refsArray.map((idx) => resolveItemFromData(dataValues, idx)).filter(Boolean);
}
...
function resolveNodeObject(nodeData, keys) {
  ...
  for (const key of Array.isArray(keys) ? keys : [keys]) {
    ...
    if (typeof idx === "number" && idx >= 0 && idx < nodeData.values.length) result[key] = nodeData.values[idx];
  }
  return result;
}
...
function normalizeVariantKey(value) {
  ...
  if (normalized.includes("dub") || normalized.includes("lat") || normalized.includes("latin") || normalized.includes("esp")) return "DUB";
  return "SUB";
}
...
function parseEmbedsFromNode(nodeData) {
  if (!nodeData || !nodeData.schema) return { SUB: [], DUB: [] };
  const schema = nodeData.schema;
  const data = nodeData.values;
  const embedsIdx = schema["embeds"];
  if (typeof embedsIdx !== "number") return { SUB: [], DUB: [] };
  const embedsRaw = data[embedsIdx];
  if (!embedsRaw || typeof embedsRaw !== "object") return { SUB: [], DUB: [] };
...
function parseDownloadsFromNode(nodeData) {
  ...
  if (typeof downloadsIdx !== "number") return { SUB: [], DUB: [] };
  const downloadsRaw = data[downloadsIdx];
  if (!downloadsRaw || typeof downloadsRaw !== "object") return { SUB: [], DUB: [] };
  const result = { SUB: [], DUB: [] };
...
function parseMediaFromNode(nodeData) {
  if (!nodeData || !nodeData.schema) return null;
  const data = nodeData.values;
  ...
  const get = (key) => { const idx = schema[key]; return (typeof idx === "number" && idx < data.length) ? data[idx] : null; };
  ...
  return { id: get("id"), title: get("title"), slug: get("slug"), synopsis: get("synopsis"), status: get("status"), episodesCount: get("episodesCount"), score: get("score"), votes: get("votes"), malId: get("malId"), startDate: get("startDate"), poster: get("poster"), category, genres, episodes };
}

function buildEpisodesList(mediaData) {
  if (!mediaData || !mediaData.episodes || !Array.isArray(mediaData.episodes)) return [];
  const slug = mediaData.slug || "";
  return mediaData.episodes.filter((ep) => ep && ep.number && ep.id).map((ep) => ({ id: ep.id, number: ep.number, title: `Episodio ${ep.number}`, url: `https://${DEFAULT_DOMAIN}/ver/${slug}-${ep.number}` }));
}
...
async function searchAnime(query, domainCandidate) {
  ...
  if (catalogNode) {
    ...
    if (Array.isArray(rawResults)) {
      results = resolveResultsArray(catalogNode.values, rawResults).filter((item) => item && item.title).map((item) => ({
        id: item.id || null, title: item.title, slug: item.slug || null,
        url: `https://${domain}/media/${item.slug}`,
        image: item.id ? `https://cdn.hentaila.com/covers/${item.id}.jpg` : null,
        backdrop: null, type: item.category ? item.category.name : null, score: item.score || null, status: item.status || null, year: null,
      }));
    }
  }
  return { success: true, data: { query: cleanQuery, results, count: results.length }, source: "hentaila" };
}
...
async function getAnimeInfo(urlCandidate) {
  ...
  if (!mediaNode) throw new ApiError(404, "Anime no encontrado en HentaiLA");
  const mediaData = parseMediaFromNode(mediaNode);
  const episodes = buildEpisodesList(mediaData);
  return {
    success: true,
    data: {
      id: mediaData.id || null, title: mediaData.title, titleJapanese: null, description: mediaData.synopsis || null,
      image: mediaData.id ? `https://cdn.hentaila.com/covers/${mediaData.id}.jpg` : null,
      backdrop: null, status: null, type: mediaData.category ? mediaData.category.name : null,
      year: mediaData.startDate ? String(mediaData.startDate).split("-")[0] : null,
      startDate: mediaData.startDate || null, endDate: null, score: mediaData.score || null, votes: mediaData.votes || null,
      totalEpisodes: episodes.length, malId: mediaData.malId || null, trailer: null,
      genres: Array.isArray(mediaData.genres) ? mediaData.genres.map((g) => ({ id: g.id || null, name: g.name, slug: g.slug || g.name.toLowerCase().replace(/\s+/g, "-"), malId: g.malId || null })) : [],
      episodes,
    },
    source: "hentaila",
  };
...
async function getEpisodeLinks(urlCandidate) {
  ...
  if (!slug || !episodeNumber) throw new ApiError(400, "URL invalida - no se pudo extraer slug y numero de episodio");
  ...
  for (let i = parsedNodes ? parsedNodes.length - 1 : 0; i >= 0; i--) {
    ...
    const nodeData = node.values;
    for (const [key, idx] of Object.entries(node.schema)) {
      if (typeof idx !== "number") continue;
      if (key === "embeds" || key === "downloads" || key === "episode") { episodeNode = node; break; }
      if (typeof nodeData[idx] === "object" && nodeData[idx] !== null && !Array.isArray(nodeData[idx])) {
        const inner = nodeData[idx];
        if (inner.embeds !== undefined || inner.downloads !== undefined || inner.episode !== undefined) {
          episodeNode = { schema: inner, values: nodeData };
          break;
        }
      }
    }
  ...
  if (!episodeNode) {
    return {
      success: true,
      data: { id: null, episode: episodeNumber, title: `Episodio ${episodeNumber}`, season: null, variants: { SUB: 0, DUB: 0 }, publishedAt: null, servers: { sub: [], dub: [] }, streamLinks: { SUB: [], DUB: [] }, downloadLinks: { SUB: [], DUB: [] } },
      source: "hentaila",
    };
  }
  ...
  return {
    success: true,
    data: {
      id: null, episode: episodeNumber,
      title: epData ? epData.title || `Episodio ${episodeNumber}` : `Episodio ${episodeNumber}`,
      season: null,
      variants: { SUB: embeds.SUB.length > 0 || downloads.SUB.length > 0 ? 1 : 0, DUB: embeds.DUB.length > 0 || downloads.DUB.length > 0 ? 1 : 0 },
      publishedAt: null,
      servers: { sub: embeds.SUB.map((l) => ({ server: l.server, url: l.url })), dub: embeds.DUB.map((l) => ({ server: l.server, url: l.url })) },
      streamLinks: { SUB: embeds.SUB.map((l) => ({ server: l.server, url: l.url })), DUB: embeds.DUB.map((l) => ({ server: l.server, url: l.url })) },
      downloadLinks: {
        SUB: downloads.SUB.map((l) => ({ server: l.server, url: l.url, quality: l.quality || l.server })),
        DUB: downloads.DUB.map((l) => ({ server: l.server, url: l.url, quality: l.quality || l.server })),
      },
    },
    source: "hentaila",
  };
